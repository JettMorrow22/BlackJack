...
public class Game {
    ...
    /**
     * Cycle of player playing their hand
    ...
    public void bettingCycle() {
        ...
        while (playAgain && player.getBalance() > 0) {
            ...
            if (player.allHandsBusted()) {
                ...
                while(calculateHand(dealer.getHand().getHand()) < 17) {
                    //give dealer new card
                    dealer.addToHand(deck.deal());
                    //method checks if busts, if so, checks for ace and
                    //cahnges to soft
                    didBust(dealer.getHand().getHand());
                }
                ...
                for ( int x = 0; x < player.getHands().size(); x++) {       
                    ...
                    if (calculateHand(dealer.getHand().getHand()) > 21) {
                    ...
                    if(calculateHand(player.getHand(x).getHand()) > 
                            calculateHand(dealer.getHand().getHand()) ) {
                        //player wins
                        player.increaseBalance(player.getHand(x).getBetAmount());
     ...
     * @param hand
     */
    public void wantToDouble (Hand hand) {
      //onlly show one of dealer cards
        System.out.println("Dealer Has: " + dealer.showOneCard());
        for (int x = 0; x < player.getHands().size(); x++) {
            System.out.println("\nPlayer Has: " + player.getHand(x).showHand()
    ...
    public void showFullTable() {
        System.out.println("Dealer Has: " + dealer.getHand().showHand() 
            + " - Total: " + calculateHand(dealer.getHand().getHand()));
